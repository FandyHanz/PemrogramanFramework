    ...
    const handleSubmit = async (event: any) => {
        ...
        try {
            ...
            if (!res?.error) {
                ...
                setIsLoading(false);
                setError(res.error === "CredentialsSignin" ? "Email or Password wrong" : res.error);
            }
        } catch (error) {
            setIsLoading(false);
            ...
            <div className={style.login__form}>
                ...
                <form onSubmit={handleSubmit}>
                    <div className={style.login__form__item}>
                        ...
                        <input type="email" id="email" name="email" placeholder="Email" className={style.login__form__item__input} required />
                    </div>

                    <div className={style.login__form__item}>
                        <label htmlFor="password" className={style.login__form__item__label}>Password</label>
                        <input type="password" id="password" name="password" placeholder="Password" minLength={6} className={style.login__form__item__input} required />
                    </div>

                    <button type="submit" disabled={isLoading} className={style.login__form__button}>
